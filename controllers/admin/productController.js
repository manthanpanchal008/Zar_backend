...
function getMakingTypeShortCode(name) {
  const norm = String(name || '').trim().toUpperCase();
...
function buildPayload(body, imageNames) {
  return {
    ...
    making_type_id: parseIntOrNull(body.making_type_id),
...
async function validatePayload(payload, productId = null) {
  ...
  if (!payload.making_type_id) return 'Please select a Making Type.';
  if (!payload.title) return 'Product title is required.';
  if (!payload.collection_name) return 'Collection name is required.';
  ...
  const { findMakingTypeById } = require('../../models/makingTypeModel');

  const [category, goldType, makingType] = await Promise.all([
    findCategoryById(payload.category_id),
    findGoldTypeById(payload.gold_type_id),
    findMakingTypeById(payload.making_type_id),
  ]);

  if (!category) return 'Selected Category is not available.';
  if (!goldType) return 'Selected Gold Type is not available.';
  if (!makingType) return 'Selected Making Type is not available.';
  ...
  const makeCode = getMakingTypeShortCode(makingType.name);
...
async function generateSku(req, res) {
  ...
  const makingTypeId = parseIntOrNull(req.query.making_type_id);
  const productId = req.query.product_id ? parseIntOrNull(req.query.product_id) : null;

  if (!categoryId || !goldTypeId || !makingTypeId) {
    return res.status(400).json({ success: false, error: 'Category, Gold Type, and Making Type are required.' });
  }
  ...
  try {
    ...
    const [category, goldType, makingType] = await Promise.all([
      ...
      findMakingTypeById(makingTypeId),
    ]);
    ...
    if (!makingType) return res.status(400).json({ success: false, error: 'Making Type not found.' });
